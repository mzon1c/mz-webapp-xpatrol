...
include("auth.php");
 $connect = Connect();
 $output = '';  
 $sql = "SELECT * FROM fine_ins where fine_ins.rfid_1 like '".$_SESSION['username']."' UNION SELECT * from fine_tax where fine_tax.rfid_1 like '".$_SESSION['username']."' UNION select * from fine_pol where fine_pol.rfid_1 like '".$_SESSION['username']."' UNION select id,rfid_1,rfid_2,date(date) as date,time(date) as time,name,remarks,fined_by from fine_speed where fine_speed.rfid_1 like '".$_SESSION['username']."'";  
